...
void readdir(TDirectory *dir, optutl::CommandLineParser parser, char TreeToUse[], int recoilType, int doES, int isWJets, int metType) 
{
  ...
  while ((key = (TKey*)next())) {
    ...
    if (obj->IsA()->InheritsFrom(TDirectory::Class())) {
      ...
      if (std::string(key->GetName()).find("_tree")){ readdir(subdir,parser,TreeToUse,parser.doubleValue("recoilType"),parser.doubleValue("doES"),
            parser.doubleValue("isWJets"),parser.doubleValue("metType"));
      }
...
*/
      ...
      for(Int_t i=0;i<t->GetEntries();++i){
         ...
         if(channel=="mt"||channel=="et"){
           ...
           std::cout<< "evt: "<<evt<<" run: "<<run<<" lumi: "<<lumi<< " pt1 " << pt1 << " mass1 " << mass1 << " pt2: "<< pt2<< " mass2: "<< mass2 <<" covmet00: "<<pfCovMatrix00<<" covmet10: "<<pfCovMatrix10<<" covmet01: "<<pfCovMatrix01<<" covmet11: "<<pfCovMatrix11<<" met x: "<<metcorr_ex<<" met cor y: "<<metcorr_ey<<std::endl;        
           //modified
...
std::cout<<"Tau pt UP: "<<pt2_UP<<" MET X UP: "<<metcorr_ex_UP<<" MET Y UP: "<<metcorr_ey_UP<<std::endl;
...
std::cout<<"Tau pt DOWN: "<<pt2_DOWN<<" MET X DOWN: "<<metcorr_ex_DOWN<<" MET Y DOWN: "<<metcorr_ey_DOWN<<std::endl;
